...
public class AwkParser {
	...
	AST SYMBOL(boolean not_in_print_root, boolean allow_in_keyword) throws IOException {
		...
		if (id_token == _EXTENSION_) {
			...
			return new ExtensionAst(extension_keyword, params);
	...
	private final class PrintAst extends ScalarExpressionAst {
		...
		private PrintAst(AST expr_list, int output_token, AST output_expr) {
	...
	private final class ExtensionAst extends AST {
		...
		private ExtensionAst(String extension_keyword, AST param_ast) {
			super(param_ast);
	...
	private final class PrintfAst extends ScalarExpressionAst {
		...
		private PrintfAst(AST expr_list, int output_token, AST output_expr) {
			super(expr_list, output_expr);
			this.output_token = output_token;
		}
	...
	private class AwkSymbolTableImpl {
		...
		private IdAst getID(String id) {
			...
			if (func_name == null) {
				...
				if (set != null && set.contains(id)) {
					...
					if (map == null) {
						local_ids.put(func_name, map = new HashMap<String, IdAst>());
					}
				} else {
					map = global_ids;
				}
		...
		int addFunctionParameter(String func_name, String id) {
			...
			Map<String, IdAst> map = local_ids.get(func_name);
			if (map == null) {
				local_ids.put(func_name, map = new HashMap<String, IdAst>());
			}
			assert map != null;
		...
		IdAst getFunctionParameterIDAST(String func_name, String f_id_string) {
		...
		AST addFunctionDef(String func_name, AST param_list, AST block) {
			...
			FunctionDefAst function_def = new FunctionDefAst(func_name, param_list, block);
