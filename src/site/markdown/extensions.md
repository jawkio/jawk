...
- `strftime([format [, timestamp [, utc-flag]]])` formats a timestamp with the C `strftime(3)` conversion specifiers (C-locale English names); the format defaults to `PROCINFO["strftime"]` or gawk's `"%a %b %e %H:%M:%S %Z %Y"`
